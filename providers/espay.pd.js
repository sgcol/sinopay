...
const signDone=(obj)=>{
    ...
    var hash=crypto.createHash('sha256');
    hash.update(('##'+[signatureKey, rq_uuid, rs_datetime, error_code, '1', 'PAYMENTREPORT-RS'].join('##')+'##').toUpperCase());
    obj.signature=hash.digest('hex');
    return obj;
}
...
const verifyDoneSign=(obj)=>{
    ...
    var hash=crypto.createHash('sha256');
    var str=('##'+[signatureKey, rq_datetime, order_id, 'PAYMENTREPORT'].join('##')+'##').toUpperCase();
    debugout(str);
    hash.update(str);
    var sign=hash.digest('hex');
    debugout(sign);
