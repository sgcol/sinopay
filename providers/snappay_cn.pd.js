...
const pify=require('pify'),getMerchant=pify(require('../merchants.js').getMerchant);
...
const makeSign=function(data, key, options) {
    delete data.sign;
    var message ='', o=Object.assign({input_charset:'UTF-8', version:'N2', service:'push', sign_type:'MD5', isShareMerchant:'true', business_time:timestring(new Date())}, data);
    Object.keys(o).sort().map((key)=>{
        if (key=='sign') return;
        if (key=='sign_type' && ((!options) || (!options.includeSignType))) return;
        message+=''+key+'='+o[key]+'&';
    })
    var encoded_sign=md5(message.substr(0, message.length-1)+key);
    o['sign'] = encoded_sign;
    return o;
}
...
exports.bestPair=(money, cb)=>{
	return cb(null, -1, 'USD');
};
exports.router=router;
exports.name='CanadaSP';
exports.options=[{name:'充值通道', values:['wxNativePay', 'aliPayN', 'wxBarCodePay', 'aliBarCodePay', 'wxJsPay']}];

exports.scanMode=function(merchant, host) {
    var tongdao=merchant.providers.snappay['充值通道']||'aliPayN';
    if (tongdao=='wxBarCodePay' || tongdao=='aliBarCodePay') return url.resolve(host, '../pvd/snappay/scanOrder');
    return null;
}
		...
		async.parallel([
            function getAccount(callback) {
                if (process.env.NODE_ENV!='production') {
                    return callback(null, {partner:'900010000018964', privateKey:'GeEhADcZl75ycvV80yuQ', merchant_id:'900010000018964'});
                }
			...
			function getSetting(cb) {
				db.settings.findOne({_id:'snappay_cn'}, (err, r)=>{
					cb(err, r||{});
				})
            }
		],
...
function init(err, db) {
	...
	order =function(orderid, money, merchantdata, mer_userid, coinType, _host, callback) {
        queryRate(account, (err, rates)=>{
            ...
            var data = {
                ...
                'pay_type' : tongdao,
                'order_desc' : '商品',
                'amount' : Math.floor(money*100/rates.USD),
                'notify_url' : url.resolve(_host, '../pvd/snappay/done'),
                'returnUrl' : url.resolve(_host, '../pvd/snappay/return'),
            };            
            var request_url = 'https://pay.foreseenewmedia.com/new_gateway.do';
            request.post({url:request_url, form:makeSign(data, account.privateKey)}, async (err, header, body)=>{
                var xmlparse=new XmlParse({explicitArray:false, strict:false}).parseStringPromise;
                var data=await xmlparse(body);
                data=data.DATA;
                if (data.IS_SUCCESS=="F") return callback(data.ERROR_MSG);
                // result 为可以跳转到支付链接的 url
                if (data.RESPONSE.ERRCODE!='00') return callback(data.RESPONSE.ERRCODEDESC);
                updateOrder(orderid, {status:'待支付', providerOrderId:data.RESPONSE.ORDERNO, snappay_account:account, lasttime:new Date(), snappay_data:data});
                ...
                var ret={url:data.RESPONSE.CODESTR};
                ret.pay_type={'aliPayN':'alipay', 'wxNativePay':'wechat'}[tongdao];
                callback(null, ret);
            })    
        });
    }

    async function scanOrder(req, res, orderid, code, callback) {
        try {
        var merchantData, money, mer_userid, cb_url, return_url, rates;
        ...
        await Promise.all([
            (async function getOrderData() {
                var results=await pify(getOrderDetail, {multiArgs:true})(orderid);
                var merchantid;
                [merchantid, money, mer_userid, cb_url, return_url] =results;
                merchantData=await getMerchant(merchantid);
            })(),
            (async function getRate() {
                rates=await pify(queryRate)(account);
            })()
        ]);
        var tongdao=merchantData.providers.snappay['充值通道']||'wxBarCodePay';
        var data = {
            'partner':account.partner,
            'merchant_id' : account.merchant_id,
            'order_id' : orderid,
            ...
            'amount' : Math.floor(money*100/rates.USD),
            'authCode' : code,
            'notify_url' : url.resolve(basepath, '../pvd/snappay/done'),
            'returnUrl' : return_url||url.resolve(basepath, '../pvd/snappay/return'),
        };            
        var request_url = 'https://pay.foreseenewmedia.com/new_gateway.do';
        var rets=await pify(request.post, {multiArgs:true})({url:request_url, form:makeSign(data, account.privateKey)});
        const [, body]=rets;
        var xmlparse=new XmlParse({explicitArray:false, strict:false}).parseStringPromise;
        var data=await xmlparse(body);
        data=data.DATA;
    ...
    router.all('/scanOrder', httpf({orderid:'string', code:'string', callback:true}, function(orderid, code, callback) {
        scanOrder(this.req, this.res, orderid, code, callback);
    }));
